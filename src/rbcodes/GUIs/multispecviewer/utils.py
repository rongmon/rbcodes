...
def show_help_dialog(parent=None):
    ...
    sections = {
        ...
        "Quick Fit": [
            ...
            ("",       "Both fits: click y-position sets linear continuum through the two points"),
            ("",       "Absorption/emission detected automatically from residual sign"),
            ("",       "If linelist is set: identifies nearest line, updates z in widget"),
            ("",       "If no linelist: reports observed centroid, FWHM/σ only"),
            ("Z",      "Revert z to previous value (press again to toggle back)"),
            ("R",      "Clear fit overlay (same key that clears line markers)"),
            ("r",      "Full reset — also clears fit overlay"),
        ],
        "Advanced Fit": [
            ("G",          "Open Advanced Fit dialog for the cursor panel"),
            ("",           "Fits 1–5 Gaussian components with a shared redshift z ± error"),
            ("Shift+C",    "(inside dialog canvas) Set z_guess from cursor / Ion 1 position"),
            ("d → d",      "(inside dialog canvas) Define two manual continuum anchor points"),
            ("x / X",      "(inside dialog canvas) Set left / right wavelength limit"),
            ("[ / ]",      "(inside dialog canvas) Pan left / right"),
            ("t / b",      "(inside dialog canvas) Set y max / min"),
            ("r",          "(inside dialog canvas) Reset canvas view"),
            ("Smooth",     "Kernel spinbox in dialog smooths display before fitting"),
            ("Advanced…",  "Edit parameter bounds, tie sigmas, or fix z"),
            ("Apply",      "Push fitted z + linelist back to the main widget"),
            ("Add absorb.","Add fitted z + linelist to the Absorber Manager"),
            ("Copy/Export","Copy results to clipboard or save to text file"),
        ],
        "vStack": [
